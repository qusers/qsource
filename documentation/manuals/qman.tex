%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Main Latex Settings for Document
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[h]
\begin{center}
\resizebox{8cm}{!}{\includegraphics[scale=0.7]{\dirfig/qlogo.png}}
\end{center}
\end{figure}
\begin{center}
%\Huge{M}\huge{ANUAL}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{PREFACE}
Q started its development sometime in the nineteen nineties originally
...
Paul Bauer, Alexandre Barrozo, Masoud  Karemi, Irek Szeler, and \AA ke
...
Q~\cite{Marelius:1999} is a set of tools tailored specifically to the
...
The main  features which have been  added to the Q  code since version
5.0 are:
...
Q 5.7 can  be compiled and run  in Windows 7, Mac OSX  and Linux.  The
parallel  \textbf{qdynp}   version  of   the  main   dynamics  program
\textbf{qdyn} has been greatly improved from that of version 5.0 which
...
are, in general, modest.  A system of  18 Å radius with a cutoff of 10
Å  in non-bonded  interactions uses  about  3 to  7 Mb  of RAM  memory
...
\url{http://xray.bmc.uu.se/~aqwww/q}, are required to use Q:
\begin{enumerate}
...
programs.  The main  program is  \textbf{qdyn} which  carries out  the
...
preparation  program \textbf{qprep}  which is  an interactive  program
that uses  pdb (Protein Data  Bank) coordinate files together  with FF
specific files  to generate the  topology. \textbf{qprep} can  also be
...
parameters for  the different states in  perturbation simulations. For
analysis of the computations the main tool is \textbf{qfep} which is a
program that  carries out  FEP and EVB  calculations of  free energies
using energy data  produced by \textbf{qdyn}. A  number of utility
programs  (trajectory   and  energy  averaging,   radial  distribution
functions, ...) are  also provided. The general outline of  Q is shown
...
of steps for preparing a topology file with \textbf{qprep} and then
describe in detail the input data required for running MD
simulations with \textbf{qdyn}.
...
\subsubsection{\textbf{qprep} - Preparing coordinates} The PDB file format
\cite{pdb} used for co-ordinate input to \textbf{qprep} is a fixed format,
...
records are read by \textbf{qprep}, all other records are ignored.

Hydrogen atoms are not required - their coordinates will be
generated by \textbf{qprep} in such a way that the bond and angles to the
...
\textbf{qprep}, the program will identify the boundaries between molecules
...
\textbf{qprep} will renumber all atoms in a single sequence starting at
...
PDB file read by \textbf{qprep} and PDB files generated by \textbf{qprep} will be
...
\textbf{qprep}. Residue numbers must be numeric, alphanumeric identifiers
...
libraries and force field parameters for AMBER95, AMBER/OPLS, OPLS-AA,
...
Load first the standard library and then your own into \textbf{qprep}. If
...
interactively. The first thing you need to learn about \textbf{qprep} is
...
A good deal of effort has been spent to make \textbf{qprep} handle errors
...
when running dynamics with \textbf{qdyn}.
...
the \textbf{qprep} preference value \textbf{solvent{\_}pack}.
...
Otherwise \textbf{qprep} will mark the topology as mixed-solvent, which is
not implemented in \textbf{qdyn} at present.
...
is correct. Fortunately \textbf{qprep} offers a number of commands to check
the topology:
...
number of parameters that affect the operation of \textbf{qprep}, $e.g.$
...
by experience users by the preference mechanism in \textbf{qprep}.
...
The idea behind this definition of different states is that \textbf{qdyn},
...
$\lambda $ given to each (pure) state in the \textbf{qdyn} input file. The
free energy differences between FEP/EVB states can then easily be
calculated by \textbf{qfep} using the standard FEP formula or the potential
of mean force (umbrella sampling) approach to obtain the EVB
ground state reaction free energy profiles.

The FEP file has the same overall structure as the \textbf{qdyn} input file
...
The part of Q that can be run in parallel is \textbf{qdyn} which contain
...
Some properties of the force fields available for Q are given in table
...
\begin{itemize}
\item \textbf{qprep} will only accept and read ATOM and HETATM records. All other record types have to be removed.
\item Residue numbers must be numeric, $i.e.$ alphanumeric residue identifiers like 60B are not allowed. Use the renumber script to renumber residues. The numbering does not have to start at 1, but \textbf{qprep} will renumber residues starting at 1.
...
\item Atom numbering is not significant and \textbf{qprep} will renumber atoms starting from 1. Note that the numbering will change due to the insertion of hydrogen atoms.
...
\item Temperature factors and occupancy numbers are ignored by \textbf{qprep} and are optional.
\end{itemize}
...
etc., and is used by \textbf{qprep} to generate a molecular topology from a
...
\textbf{qprep}. The [\textbf{atoms}] section defines the sequential number,
...
\hline 2 &  Lennard-Jones A parameter for type 1 pairs (kcal$^{1/2}$ $\cdot$ mol$^{-1/2}$ $\cdot$ Å$^{6}$)
for geometric combination or R$^*$ (kcal$\cdot$mol$^{-1}\cdot$Å$^{12}$) for arithmetic combination) \\
\hline 3 & LJ A parameter type 2 (kcal$^{1/2}$ $\cdot$ mol$^{-1/2}$ $\cdot$ Å$^{6}$) or R$^*$ (kcal$\cdot$mol$^{-1}\cdot$Å$^{12}$)\\
\hline 4 &  LJ B parameter type 1 (kcal$^{1/2}$ $\cdot$ mol$^{-1/2}$ $\cdot$ Å$^{3}$) or $\varepsilon$ (kcal$\cdot$mol$^{-1}\cdot$Å$^{6}$)\\
\hline 5 & LJ A parameter type 3 (kcal$^{1/2}$ $\cdot$ mol$^{-1/2}$ $\cdot$ Å$^{6}$) or R$^*$ (kcal$\cdot$mol$^{-1}\cdot$Å$^{12}$)\\
\hline 6  & LJ B parameter type 2 and 3 (kcal$^{1/2}$ $\cdot$ mol$^{-1/2}\cdot$Å$^{3}$) or $\varepsilon$(kcal$\cdot$mol$^{-1}\cdot$Å$^{6}$)\\
...
of this shell and the force constant is given in the \textbf{qdyn} input
...
been implemented in \textbf{qdyn} according to equation \ref{eq:softcore}.
...
\textbf{qdyn} in a pairwise manner based upon the desired potentials at
