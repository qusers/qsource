...
!
! calc_nb.f90
! by Martin Alml�f & Martin Nervall
!       
...
671	format('radius',i2 ,' : ',f9.2)
do j = 1, 40
	write (*,671) j, ave_elj_r(i,j)
end do
...
subroutine calc_center()
	integer, allocatable	::	group1(:)
	real					::	sum_center_mask1(3)
	integer					:: nat1, j, l
	...
	nat1 = masks(1)%included 
	allocate(group1(nat1))

		sum_center_mask1(:) = 0.0_8
	l = 1
	do j = 1, nat_pro
		if (masks(1)%MASK(j)) then
			group1(l) = j
			l = l+1
		end if
	end do
	...
	do j = 1, nat1
		...
		sum_center_mask1(1) =sum_center_mask1(1)+ xin(3*group1(j)-2)
		sum_center_mask1(2) =sum_center_mask1(2)+ xin(3*group1(j)-1)
		sum_center_mask1(3) =sum_center_mask1(3)+ xin(3*group1(j))

	end do

	center_mask1(:) = sum_center_mask1(:)/nat1

!		sum_center_mask1(2) += xin(3*group1(j)-1)
!		sum_center_mask1(3) += xin(3*group1(j))
...
integer function dist_group1(atom2)
	integer :: atom2
	real :: x1, x2 ,y1, y2, z1, z2
		...
		dist_group1 = floor(sqrt((x1-x2)**2+(y1-y2)**2+(z1-z2)**2))

	


end function dist_group1
...
!	Make averages. To be printed after all trajectory files are processed
	...
	ave_Elj_r(i,:) = (ave_Elj_r(i,:)*total_frames(i) + Elj_r(:))/(total_frames(i)+1)
	...
	do j=1,storleken
		...
		if (r /= 0) then
			...
			if(ivdw_rule==1) then !geometric comb. rule
				Elj_r(dist_group1(nb_list%atom2(j))) =Elj_r(dist_group1(nb_list%atom2(j)))+ &
					nb_list%AA(j)*invr12 - nb_list%BB(j)*invr6
				NB_Vlj = NB_Vlj + nb_list%AA(j)*invr12 - nb_list%BB(j)*invr6
            else !arithmetic
				Elj_r(dist_group1(nb_list%atom2(j))) =Elj_r(dist_group1(nb_list%atom2(j)))+ &
					sqrt(nb_list%BB(j)) * (nb_list%AA(j))**6 * invr6 * ((nb_list%AA(j))**6 * invr6 - &
					2.0)
				NB_Vlj = NB_Vlj + sqrt(nb_list%BB(j)) * ( nb_list%AA(j)**6 * invr6 * &
					( nb_list%AA(j)**6 * invr6 - 2.0 ) )
			endif			
