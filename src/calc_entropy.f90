...
20  format('Entropy calculation for ',i6,' atoms')
  ...
  !------------------------------------------------------------------------------!
  !!  subroutine: construct_trajectory
  !!
  !------------------------------------------------------------------------------!
  ...
  subroutine entropy_initialize
    ...
    kb=1.38066                                                                              ! Boltzmann constant  *10**(-23)
    hs=1.0545727                                                                            ! Plancks constant     *10**(-34)
...
subroutine multiplymatrices(t)
  ...
  c=(e**2)*kb*(t%Temperature)*u*0.01/(hs**2)        ! 0.01 is � to m, u is u to kg (hs*hs) to hs**2
...
subroutine entropy_ho(m, t, det)
  ...
  do i= 1,no_freq
    ...
    w = sqrt(kb*t%Temperature/(u*t%VIZ(k)))    ! Removed -1.0 since i don't have to shift  ! 10**(12) = sqrt(10**(-23, kb)/(10**(-27, u)*10**(-20, �**2 to m**2))
