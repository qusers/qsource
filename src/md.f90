...
#include "mpif.h"
  ...
  !variables used when user change boxsize in inputfile
  logical                   :: control_box
  real(8)                   :: new_boxl(3) 
...
9       format ('Pressure = ',f10.3,'  bar')
    ...
    write (*,10) nsteps
10  format ('Number of MD steps  =',i10)
    write (*,11) stepsize
11  format ('Stepsize (fs)       =',f10.3)
...
12  format ('Target temperature  =',f10.2)
13  format ('T-relax time        =',f10.2)
    ...
    write(*,'(a)') 'Initial velocities will be generated from Maxwells distribution'
    if (iseed > 0) write (*,15) Tmaxw
    if (iseed > 0) write (*,16) iseed
...
25  format ('Cut-off radii for non-bonded interactions:',/, &
      'Solute-solute       =',f10.2,/,&
      'Solvent-solvent     =',f10.2,/,&
      'Solute-solvent      =',f10.2,/,&
      'Q-atom-non-Q-atom   =',f10.2)
26  format ('LRF                 =',f10.2)
...
50      format('Radius of inner restrained shell       =',f8.2)
...
65    format ('Initial coord. file= ',a)
      ...
      if(.not. prm_get_string_by_key('final', xfin_file)) then
        write(*,'(a)') '>>> ERROR: final coordinate file not specified (section files, keyword final)'
        initialize = .false.
      end if
