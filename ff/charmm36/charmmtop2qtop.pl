#!/usr/bin/perl
# Script to translate charmm topologies to q topologies.
# Adopted by: Mauricio Esguerra
# Original: Unknown original author
# Date: March 18, 2014
use strict; 
use warnings;
...
while (<IN>) {
   ...
   if ($_ =~ /^(IMPR)\s+(\w+)\s+(\w+)\s+(\w+)\s+(\w+)\s*(\w+)?\s*(\w+)?\s*(\w+)?\s*(\w+)?\s*(\w+)?\s*(\w+)?\s*(\w+)?\s*(\w+)?\s/ and $activate_first) {
