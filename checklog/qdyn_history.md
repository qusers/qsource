...
- The sizes.f90 file now includes definitions for single and quadruple
  precision for reals as well as type definitions for integers. The
  old definition *wp8* has been redefined as *dp* complying with
  standard usage.

- Documentation for developers is now handled with doxygen. 
...
## Release 3.52, 99-01-28
    ...
    Exclusion of waters now only outside rwat+2Å to avoid excluding waters from a 
    previous simulation
...
## Intermediate version 3.19, 97-11-27
    Changed water polarisation restraints according to JÅ's new model.
