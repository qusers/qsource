...
================================================================================
 File:        charmm2q.py
 Authors:     Geir V. Isaksen with tiny modifications by Mauricio Esguerra and
              Christoffer Lind
 Date:        March 9, 2016
 Email:       geir.isaksen@gmail.com

 Description:
 This is a general translator of the charmm36 force-field to the format needed
 for the Q program.
...
def read_Q_prm(filename):
    """
    Reads original Q prm file and collects existing atomtypes for parameters.
    Returns dictionary.
...
def read_charmm_par(filename, atom_mass, existing_prm, convert_to_opls):
    """
    Reads a charmm parameter file and returns a dictionary with parameters
    suitable for Q.
    If a Q parameter file is specified, non-existing parameters will be returned.
    Else all parameters in the file will be returned.
    """
                ...
                if not data.startswith('!') and len(data.split()) > 6:
                    ...
                    try:
                        if types not in existing_prm['[impropers]']:
                            ...
                            # Notice that the order here is inverted to keep with Q convention
                            params['[impropers]'].append('%6s %6s %6s %6s %9.3f %9.3f %s' %
                                                         (a2.ljust(6), a1.ljust(6), a4.ljust(6), a3.ljust(6), k, imp,
                                                          comment))
                    except:
                        pass
...
def write_file_header(filename, ff_name, scale_14, convert_to_opls):
    """
    ...
    qprm.write('*-------------------------------------------------------------------------------\n'
               ...
               '*-------------------------------------------------------------------------------\n'
               '*%s\n'
               '* Parameters translated for Q with charmm2q.py (Geir V. Isaksen, 2016)\n'
               '* with small changes by Mauricio Esguerra and Christoffer Lind.\n'
               '*-------------------------------------------------------------------------------\n'
...
def write_Q_prm(filename, new_prm):
    """
    ...
    headers = {1: '* V(Lennard-Jones) = Eps,i,j[(Rmin,i,j/ri,j)**12 - 2(Rmin,i,j/ri,j)**6]\n'
               ...
               2: '* V(bond) = Kb(b - b0)**2\n'
                  ...
                  '*-------------------------------------------------------------------------------',
               3: '* V(angle) = Ktheta(Theta - Theta0)**2\n'
                  ...
                  '*-------------------------------------------------------------------------------',
               4: '* V(dihedral) = Kchi(1 + cos(n(chi) - delta))\n'
                  '* Kchi: kcal/mole\n'
                  '* n: multiplicity\n'
                  '* delta: degrees\n'
                  ...
                  '*-------------------------------------------------------------------------------',
               5: '* V(improper) = Kpsi(psi - psi0)**2\n'
                  '* Kpsi: kcal/mole/rad**2\n'
                  '* psi0: degrees\n'
                  '* note that the second column of numbers (0) is ignored\n'
                  '* atom types           Kpsi                   psi0\n'
                  '* \n'
                  '* iaci  iacj   iack   iacl      forceK     imp0\n'
                  '*-------------------------------------------------------------------------------'}
...
def write_Q_lib(filename, residues, topname):
    """
    Write out a readable Q lib file from the residues dictionary collected
    from the CHARMM top .rtf file
    """
...
#    for res in residues.keys():
    ...
    libfile.write('* Converted with charmm2q.py (Geir V. Isaksen, 2016)')
    libfile.write('* with tiny changes by Mauricio Esguerra and Christoffer Lind')
    libfile.write('\n*\n')
...
def patch_to_residue(residues, patches):
    """
    Q lib does not support patches, so we need to generate all residues.
    Too bad, but ok, here goes.
    """
    ...
    # GLY and PRO have special N-terminals, so they do not use standard
    # N-terminal for these two.
...
def patch_terminals(residues, patches):
    """
...
print '\nConvert CHARMM parameters to Q\n\n '

if len(sys.argv) > 1:
    if sys.argv[1].lower() == 'opls':
        print 'OPLS keyword invoked!'
        print 'CHARMM parameters will be attempted converted to Q OPLS format (AMBER type).'
        print 'Make sure to inspect that parameter files are correct!'
        convert_to_opls = True

charmm_par = raw_input('Specify CHARMM parameter file: ')

if not os.path.isfile(charmm_par):
    print 'Could not find CHARMM parameter file %s' % charmm_par
    sys.exit()

charmm_top = raw_input('\nSpecify CHARMM topology file: ')
if not os.path.isfile(charmm_top):
    print 'Could not find CHARMM topology file %s' % charmm_top
    sys.exit()

# Try to get force field name and version:
ff_name = find_ff_name(charmm_par)
...
print '\nAppend new parameters to existing Q parameter file? (Y/N):'
if raw_input().lower() == 'y':
    old_q_prm = raw_input('Specify Q parameter file:')

    if os.path.isfile(old_q_prm):
        qprm = read_Q_prm(old_q_prm)

        print 'New CHARMM parameters will be merged with %s' % old_q_prm
        print ''
        print '%6d ATOM TYPES READ' % len(qprm['[atom_types]'])
        print '%6d BOND PARAMETERS READ' % len(qprm['[bonds]'])
        print '%6d ANGLE PARAMETERS READ' % len(qprm['[angles]'])
        print '%6d TORSION PARAMETERS READ' % len(qprm['[torsions]'])
        print '%6d IMPROPER PARAMETERS READ' % len(qprm['[impropers]'])
        # Write new parameters to Q .prm file
    else:
        print 'Could not find Q parameter file %s' % old_q_prm

else:
    old_q_prm = 'q%s.prm' % charmm_par.split('.')[0]

    print '\nNew parameters will be written to %s' % old_q_prm


# Create Q library file?
print '\nCreate Q library file? (Y/N):'
if raw_input().lower() == 'y':
    create_lib = True


# Get atom weights from top .rtf file
atom_mass = get_atom_masses(charmm_top)

# Get new parameters from charmm .par file
new_prm, scale_14 = read_charmm_par(charmm_par, atom_mass, qprm, convert_to_opls)

write_file_header(old_q_prm, ff_name, scale_14, convert_to_opls)
...
# Write new parameters to Q .prm file
write_Q_prm(old_q_prm, new_prm)

if create_lib:
    # Get Q lib info from top .rtf file
    residues, patches = read_charmm_top(charmm_top)
    ...
    residues = patch_to_residue(residues, patches)

    # Generate all N- and C-terminals from patches
    residues = patch_terminals(residues, patches)

    # Write Q library file
    q_libfile = 'q%s.lib' % charmm_top.split('.')[0]
    top_name = find_ff_name(charmm_top)

    write_Q_lib(q_libfile, residues, top_name)
...
#         description = '       == From a CHARMM parameter and topology file \n \
#         create a translated parameter and library file to be used  \n \
#         with Q == ')
# 
#     parser.add_argument('-p',
#                         dest = "prm",
#                         required = True,
#                         help = "Name of the (.prm) parameter file")
# 
#     parser.add_argument('-t',
#                         dest = "rtf",
#                         help = "Name of the (.rtf) topology file",
#                         default = os.getcwd())
# 
# 
